...
class Gallery extends Component {
  state = {isClicked: imagesList[0].id}
  ...
  getGalleryImage = () => {
    ...
    const imageList = imagesList[isClicked]
    return imageList
  }

  render() {
    const {isClicked} = this.state
    const imageList = this.getGalleryImage()
    return (
      <div className="bg-container">
        <img
          src={imageList.imageUrl}
          alt={imageList.imageAltText}
        ...
        <ul className="images-list-container">
          {imagesList.map(eachImage => (
            <ThumbnailItem
              ...
              isActive={eachImage.id === isClicked}
